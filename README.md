...
当前骨架只提供只读状态面板和 capability 空壳。`dev:full` 与 `dev:online` 使用同一套代码，通过 Tauri 配置变体和 Rust feature 区分。
...
- Full flavor 已在 Rust 状态中标记包含本机后端，但本轮尚未打包或启动真实 `backend-all-in-one`。
