...
当前骨架提供只读状态面板、capability 状态和 Full 本机后端 sidecar 最小启动闭环。`dev:full` 与 `dev:online` 使用同一套代码，通过 Tauri 配置变体和 Rust feature 区分。
...
- 本机 token 只保存在 Rust 主进程状态中，状态面板只显示本机会话是否就绪，不返回 token 或 header。
